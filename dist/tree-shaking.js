...
class TreeShaker {
                        ...
                        if (spec.type === 'namespace') {
                            ...
                            if (depMod) {
                                for (const exp of depMod.exports) {
                                    if (this.markUsed(depPath, exp.name))
                                        changed = true;
                                }
                            }
                        }
                        else if (spec.type === 'named') {
                            if (this.markUsed(depPath, spec.imported))
                                changed = true;
                        }
                        else if (spec.type === 'default') {
                            if (this.markUsed(depPath, 'default'))
                                changed = true;
                        }
    ...
    markSideEffectFreeModules(graph) {
        const modules = graph.getModules();
        for (const mod of modules) {
    ...
    isSideEffectFree(mod, graph) {
        if (this.config.sideEffects === false)
            return true;
        ...
        if (Array.isArray(pkgSideEffects)) {
            ...
            const matches = pkgSideEffects.some(pattern => {
                const regex = new RegExp('^' + pattern.replace(/\*/g, '.*') + '$');
                return regex.test(relPath);
            });
            if (matches)
    ...
    checkPackageJsonSideEffects(filePath) {
    ...
    removeUnusedDeclarations(mod, usedExports) {
        if (mod.sideEffects)
            return mod.code;
        let result = mod.code;
        ...
        for (const name of unusedNames) {
            const exportFuncPattern = new RegExp(`export\\s+function\\s+${escapeRegExp(name)}\\s*\\([^)]*\\)\\s*\\{`, 'g');
            let match;
            ...
            const funcPattern = new RegExp(`function\\s+${escapeRegExp(name)}\\s*\\([^)]*\\)\\s*\\{`, 'g');
            ...
            const constPattern = new RegExp(`export\\s+(const|let|var)\\s+${escapeRegExp(name)}\\s*=[^;]*;\\s*`, 'g');
            result = result.replace(constPattern, '');
            const constPattern2 = new RegExp(`(const|let|var)\\s+${escapeRegExp(name)}\\s*=[^;]*;\\s*`, 'g');
            result = result.replace(constPattern2, '');
            const classPattern = new RegExp(`export\\s+class\\s+${escapeRegExp(name)}\\s*\\{`, 'g');
            ...
            const classPattern2 = new RegExp(`class\\s+${escapeRegExp(name)}\\s*\\{`, 'g');
    ...
    removeUnusedExports(mod, usedExports) {
        let code = mod.code;
        for (const exp of mod.exports) {
            if (exp.used || exp.type === 're-export' || exp.type === 're-export-all')
                continue;
    ...
    generateEmptyModule(mod) {
        if (mod.moduleType === 'cjs') {
            return 'module.exports = {};';
        }
        return '';
    }
    resolvePath(source, fromPath, graph) {
...
//# sourceMappingURL=tree-shaking.js.map