...
exports.decodeSourceMapMappings = decodeSourceMapMappings;
exports.encodeSourceMapMappings = encodeSourceMapMappings;
...
function encodeSourceMapMappings(segments) {
    const lines = [];
    let prevLine = 0;
    let prevCol = 0;
    let prevSrc = 0;
    ...
    let prevOrigCol = 0;
    let prevName = 0;
    const sorted = [...segments].sort((a, b) => a.generatedLine !== b.generatedLine
    ...
    for (const seg of sorted) {
        ...
        const fields = [seg.generatedColumn - prevCol];
...
function mergeSourceMaps(parent, child) {
    ...
    return {
        ...
        sourcesContent: sources.map(s => {
            ...
            if (ci >= 0 && child.sourcesContent?.[ci])
                return child.sourcesContent[ci];
            const pi = parent.sources.indexOf(s);
            if (pi >= 0 && parent.sourcesContent?.[pi])
                return parent.sourcesContent[pi];
            return null;
        }),
    };
}
function generateSourceMap(generatedCode, originalCode, originalFile, generatedFile) {
...
function appendSourceMapUrl(code, mapFileName) {
